<!DOCTYPE html>
<html>
<head>
	<title>Jelexie Bakeshop</title>
	<meta charset="utf-8">
	<meta name="viewport" content="width=device-width, initial-scale=1.0"/>
	<link rel="icon" href="favicon.png">
	<link href="assets/css/style.css" rel="stylesheet" type="text/css" />
	<link rel="stylesheet" type="text/css" href="assets/css/responsive.css">
	<link rel="stylesheet" type="text/css" href="assets/fonts/fontawesome/css/all.css" media="all">
	<link rel="stylesheet" type="text/css" href="assets/css/bootstrap.min.css">
	<link rel="stylesheet" type="text/css" href="assets/css/normalize.css">
</head>
<body>

<div id="fb-root"></div>
<script>(function(d, s, id) {
  var js, fjs = d.getElementsByTagName(s)[0];
  if (d.getElementById(id)) return;
  js = d.createElement(s); js.id = id;
  js.src = 'https://connect.facebook.net/en_US/sdk.js#xfbml=1&version=v3.0';
  fjs.parentNode.insertBefore(js, fjs);
}(document, 'script', 'facebook-jssdk'));</script>

<div id="main" class="main">
<?php include 'navbar.html';?>

<section class="ads">
<div class="ads">
	<img class="img-fluid" src="assets/img/morning coffee combo tarp 2018 (size 3).min.jpg">
</div>
</section>

<section id="about" class="about-section pt-3 mt-4 mb-3">
	<div class="container pb-2 mb-5">
		<div class="row">
			<div class="col-md-8">
				<h1 class="mt-0">ABOUT US</h1>
				<p>
					Behind our company’s success are different people united by the same vision — one summarized in ensuring total customer satisfaction.
					<div class="hvr-float-shadow hvr-icon hvr-icon-pulse-grow read-more float-right"><a href="#" class="">Read more <i class="hvr-icon hvr-icon-pulse-grow fa fa-angle-double-right" style="position: relative; top: -1.5px;"></i></a></div>
				</p>
			</div>

			<div class="col-md-4 mt-3 text-center">
				<div class="" style="border: 1px solid #000000; padding: 100px;">
					
				</div>
			</div>
		</div>
	</div>

	<div class="mt-2 pt-2 bg-gray">
		<div class="container-fluid pl-2 mt-2">
			<div class="row">
				<div class="col-md-6" style="border: 1px solid #000000;">
					
				</div>

				<div class="col-md-6 px-3">
					<div style="margin: auto;" class="fb-page" data-href="https://facebook.com/Jelexie-179456868470/" data-tabs="timeline, events, messages" data-width="500" data-small-header="true" data-adapt-container-width="true" data-hide-cover="false" data-show-facepile="true"><blockquote cite="https://facebook.com/Jelexie-179456868470/" class="fb-xfbml-parse-ignore"><a href="https://facebook.com/Jelexie-179456868470/">Jelexie</a></blockquote></div>
				</div>
			</div>
		</div>
	</div>
</section>

<?php include 'footer.html';?>
</div>


<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.3.1/jquery.min.js"></script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/jquery-easing/1.4.1/jquery.easing.min.js"></script>
  <script src="assets/js/bootstrap.min.js"></script>
  <script src="assets/js/jquery.fittext.js"></script>
  <script src="assets/js/jquery.sticky.js"></script>
  <script src="assets/js/custom.js"></script>
</body>
</html>